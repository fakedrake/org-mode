	...
	(let (beg end msg)
	  (cond
	   ...
	   ((let ((datum (org-element-context)))
	      (when (or (memq (org-element-type datum)
			      '(latex-environment latex-fragment))
			(and (eq (org-element-type datum) 'export-block)
...
`org-format-latex-options', which see."
  ...
  (unless (eq processing-type 'verbatim)
	...
	(unless (and overlays
	  ...
	  (let* ((context (org-element-context))
		 (type (org-element-type context)))
	    (when (or (memq type '(latex-environment latex-fragment))
		      (and (eq type 'export-block)
			   (equal (org-element-property :type context) "LATEX")))
